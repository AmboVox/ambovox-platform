...
function drawShapeOnCtx(
  ...
  switch(type){
    ...
    case'triangle':{
      ctx.beginPath();ctx.moveTo(ax+aw/2,ay);ctx.lineTo(ax+aw,ay+ah);ctx.lineTo(ax,ay+ah)
      ctx.closePath();ctx.stroke();break
    }
    case'star':{
      const scx=ax+aw/2,scy=ay+ah/2,outerR=Math.min(aw,ah)/2,innerR=outerR*0.38
      ctx.beginPath()
      for(let i=0;i<10;i++){
        const angle=(i*Math.PI/5)-Math.PI/2,r=i%2===0?outerR:innerR
    ...
    case'pentagon':{
      const pcx=ax+aw/2,pcy=ay+ah/2,r=Math.min(aw,ah)/2
      ctx.beginPath()
      for(let i=0;i<5;i++){
    ...
    case'highlight':{
      ctx.setLineDash([])
      ctx.globalAlpha=preview?0.15:0.35;ctx.fillRect(ax,ay,aw,ah);ctx.globalAlpha=1
      if(preview){ctx.lineWidth=1;ctx.strokeRect(ax,ay,aw,ah)};break
    }
    case'text':{
      ctx.setLineDash([])
      if(preview||!text){ctx.lineWidth=1;ctx.strokeRect(ax,ay,aw,ah)}
      else{const fs=Math.max(12,Math.min(ah*0.65,48));ctx.font=`bold ${fs}px sans-serif`;ctx.fillText(text,ax+4,ay+fs)};break
    }
  }
  ctx.setLineDash([])
}
...
function PaintColorPicker({value,onChange}:{value:string;onChange:(hex:string)=>void}){
  ...
  useEffect(()=>{lH.current=h},[h])
  useEffect(()=>{lS.current=s},[s])
  useEffect(()=>{lV.current=v},[v])

  function applyHSV(nh:number,ns:number,nv:number){
    const [nr,ng,nb]=hsvToRgb(nh,ns,nv),nhex=rgbToHex(nr,ng,nb)
  ...
  function applyRGB(nr:number,ng:number,nb:number){
    const [nh,ns,nv]=rgbToHsv(nr,ng,nb),nhex=rgbToHex(nr,ng,nb)
    setH(nh);setSat(ns);setV(nv);setR(nr);setG(ng);setB(nb);setHexInput(nhex);onChange(nhex)
  }
  useEffect(()=>{
    function onMove(e:MouseEvent){
      if(isDraggingGrad.current&&gradRef.current){
        ...
        applyHSV(lH.current,Math.max(0,Math.min(1,(e.clientX-rect.left)/rect.width)),Math.max(0,Math.min(1,1-(e.clientY-rect.top)/rect.height)))
      }
      if(isDraggingHue.current&&hueRef.current){
        const rect=hueRef.current.getBoundingClientRect()
        applyHSV(Math.max(0,Math.min(359.9,((e.clientY-rect.top)/rect.height)*360)),lS.current,lV.current)
      }
    }
  ...
  return(
    <div style={{userSelect:'none'}}>
      <div style={{display:'flex',gap:'8px',marginBottom:'10px'}}>
        <div ref={gradRef} onMouseDown={e=>{isDraggingGrad.current=true;e.preventDefault();if(gradRef.current){const rect=gradRef.current.getBoundingClientRect();applyHSV(lH.current,Math.max(0,Math.min(1,(e.clientX-rect.left)/rect.width)),Math.max(0,Math.min(1,1-(e.clientY-rect.top)/rect.height)))}}}
        ...
        <div ref={hueRef} onMouseDown={e=>{isDraggingHue.current=true;e.preventDefault();if(hueRef.current){const rect=hueRef.current.getBoundingClientRect();applyHSV(Math.max(0,Math.min(359.9,((e.clientY-rect.top)/rect.height)*360)),lS.current,lV.current)}}}
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'2px',flexShrink:0}}>
          ...
          <div style={{width:'30px',height:'46px',borderRadius:'0 0 4px 4px',background:value,border:'1px solid rgba(255,255,255,0.15)',opacity:0.7}} title="Previous color" />
...
// ── PreviewMirror — mirrors a live video stream onto a canvas ─────────────────
  ...
  useEffect(()=>{
    ...
    function draw(){
      ...
      if(video&&video.readyState>=2&&video.videoWidth>0){
        ...
        canvas.width=canvas.width||1280;canvas.height=canvas.height||720
        ctx.fillStyle='#1a1a2e';ctx.fillRect(0,0,canvas.width,canvas.height)
        ctx.fillStyle='rgba(255,255,255,0.2)';ctx.font='16px sans-serif';ctx.textAlign='center'
        ctx.fillText('Starting…',canvas.width/2,canvas.height/2);ctx.textAlign='left'
      }
...
export default function RecordStudioPage(){
  ...
  // Single-stream recording (webcam-only or screen-only) — unchanged, proven working
  const [videoBlob,setVideoBlob]=useState<Blob|null>(null)
  const [videoUrl,setVideoUrl]=useState<string|null>(null)

  // Dual-stream recording (webcam + screen) — recorded as two independent layers
  ...
  const [requestingLabels,setRequestingLabels]=useState(false)
  ...
  // Always-mounted source elements (bound to the raw live streams)
  const webcamRef=useRef<HTMLVideoElement>(null)
  const screenRef=useRef<HTMLVideoElement>(null)

  // Playback elements used in Review / Editing
  const mainPlaybackRef=useRef<HTMLVideoElement>(null)
  const pipPlaybackRef=useRef<HTMLVideoElement>(null)
  const shapeCanvasRef=useRef<HTMLCanvasElement>(null)

  // Single-stream recorder (webcam-only / screen-only)
  const mediaRecorderRef=useRef<MediaRecorder|null>(null)
  const chunksRef=useRef<Blob[]>([])

  // Dual-stream recorders (webcam + screen)
  ...
  async function refreshDevices(){
    try{
      ...
      setDevicesLabeled(devices.some(d=>d.label))
    }catch{}
  }

  async function requestDeviceLabels(){
    setRequestingLabels(true)
    try{
  ...
  // Reassign stream srcObjects after stage transitions (fixes remount blanking)
  useEffect(()=>{
    if(stage!=='preview'&&stage!=='recording')return
    const timer=setTimeout(()=>{
      if(webcamStreamRef.current&&webcamRef.current&&webcamRef.current.srcObject!==webcamStreamRef.current){
        webcamRef.current.srcObject=webcamStreamRef.current
        webcamRef.current.play().catch(()=>{})
      }
      if(screenStreamRef.current&&screenRef.current&&screenRef.current.srcObject!==screenStreamRef.current){
  ...
  // Poll currentTime for shape/caption sync, plus gentle pip resync
  useEffect(()=>{
    if(stage!=='editing')return
    const poll=()=>{
      if(mainPlaybackRef.current)setCurrentTime(mainPlaybackRef.current.currentTime)
      if(mode==='both'&&pipPlaybackRef.current&&mainPlaybackRef.current){
        const drift=Math.abs(pipPlaybackRef.current.currentTime-mainPlaybackRef.current.currentTime)
        if(drift>0.3)pipPlaybackRef.current.currentTime=mainPlaybackRef.current.currentTime
      }
  ...
  async function startPreview(selectedMode:RecordMode){
    ...
    try{
      if(selectedMode==='webcam'||selectedMode==='both'){
        const videoConstraint:MediaTrackConstraints|boolean=selectedCameraId?{deviceId:{exact:selectedCameraId}}:true
        const audioConstraint:MediaTrackConstraints|boolean=selectedMicId?{deviceId:{exact:selectedMicId}}:true
  ...
  async function startRecording(){
    ...
    if(mode==='both'){
      ...
      const screenRecorder=new MediaRecorder(screenStreamRef.current!,{mimeType})
      const webcamRecorder=new MediaRecorder(webcamStreamRef.current!,{mimeType})
      let screenDone=false,webcamDone=false
      function checkBothDone(){
        if(screenDone&&webcamDone){
          const sBlob=new Blob(screenChunksRef.current,{type:'video/webm'})
          const wBlob=new Blob(webcamChunksRef.current,{type:'video/webm'})
          setScreenBlob(sBlob);setScreenUrl(URL.createObjectURL(sBlob))
          setWebcamBlob(wBlob);setWebcamUrl(URL.createObjectURL(wBlob))
          setStage('review')
        }
      }
      ...
      screenRecorder.onstop=()=>{screenDone=true;checkBothDone()}
      webcamRecorder.onstop=()=>{webcamDone=true;checkBothDone()}
  ...
  function pauseRecording(){
    if(mode==='both'){
      ...
      if(sr?.state==='recording'){
        sr.pause();wr?.pause();setIsPaused(true)
        if(timerRef.current)clearInterval(timerRef.current)
      }else if(sr?.state==='paused'){
        sr.resume();wr?.resume();setIsPaused(false)
        timerRef.current=setInterval(()=>setRecordingTime(t=>t+1),1000)
      }
    }else{
  ...
  function discardRecording(){
    ...
    setScreenBlob(null);setScreenUrl(null)
    setWebcamBlob(null);setWebcamUrl(null)
  ...
  async function saveVideo(){
    ...
    if(mode==='both'&&(!screenBlob||!webcamBlob)){alert('Recording data is missing.');return}
    if(mode!=='both'&&!videoBlob){alert('Recording data is missing.');return}
    setStage('saving')
    const supabase=createClient()
    try{
      if(mode==='both'){
        const base=`${user.id}/${Date.now()}-${title.replace(/\s+/g,'-')}`
        const r1=await supabase.storage.from('lecture-videos').upload(`${base}-screen.webm`,screenBlob!,{contentType:'video/webm',cacheControl:'3600'})
        if(r1.error)throw r1.error
        const r2=await supabase.storage.from('lecture-videos').upload(`${base}-webcam.webm`,webcamBlob!,{contentType:'video/webm',cacheControl:'3600'})
      ...
      if(mode==='both'){
        ...
        const a2=document.createElement('a');a2.href=URL.createObjectURL(webcamBlob!);a2.download=`${title.replace(/\s+/g,'-')}-webcam.webm`;a2.click()
      }else{
        const a=document.createElement('a');a.href=URL.createObjectURL(videoBlob!);a.download=`${title.replace(/\s+/g,'-')}.webm`;a.click()
      }
  ...
  function handleCanvasMouseMove(e:React.MouseEvent<HTMLCanvasElement>){
    ...
    if(placingCaptionZone){
      setCaptionZone({x:Math.min(drawStart.x,pos.x),y:Math.min(drawStart.y,pos.y),width:Math.abs(pos.x-drawStart.x),height:Math.abs(pos.y-drawStart.y)})
    }
  }
  function handleCanvasMouseUp(e:React.MouseEvent<HTMLCanvasElement>){
    ...
    setEditData(prev=>({...prev,shapes:[...prev.shapes,{
      id:Date.now().toString(),type:activeShapeType,
      x:raw?drawStart.x:Math.min(drawStart.x,pos.x),y:raw?drawStart.y:Math.min(drawStart.y,pos.y),
      x2:raw?pos.x:Math.max(drawStart.x,pos.x),y2:raw?pos.y:Math.max(drawStart.y,pos.y),
  ...
  function startAutoCaption(){
    ...
    recognition.onresult=(e:any)=>{
      ...
      if(!text)return
      const start=mainPlaybackRef.current?.currentTime||0
  ...
  function resetAll(){
    ...
    setScreenBlob(null);setScreenUrl(null)
    setWebcamBlob(null);setWebcamUrl(null)
    setTitle('');setRecordingTime(0);setWasDownloaded(false)
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',minHeight:'100vh',background:'#1a1a2e'}}>
      ...
      {/* Always-present hidden source elements — srcObject never gets cleared by remounts */}
        ...
        {stage==='mode_select'&&(
          <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',padding:'40px',overflowY:'auto'}}>
            <div style={{maxWidth:'640px',width:'100%',textAlign:'center'}}>
              ...
              {/* Device picker */}
              <div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'12px',padding:'16px 20px',marginBottom:'28px',textAlign:'left'}}>
                <div style={{color:'rgba(255,255,255,0.6)',fontSize:'11px',fontWeight:'700',textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:'10px'}}>Choose Your Devices</div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:devicesLabeled?'0':'10px'}}>
                  <div>
                ...
                {!devicesLabeled&&(
                  <button onClick={requestDeviceLabels} disabled={requestingLabels} style={{marginTop:'10px',padding:'6px 14px',background:'rgba(196,122,44,0.2)',color:'#C47A2C',border:'1px solid rgba(196,122,44,0.4)',borderRadius:'6px',fontSize:'11px',fontWeight:'600',cursor:requestingLabels?'default':'pointer'}}>
                    {requestingLabels?'Requesting…':'🔓 Allow access to see device names'}
                  </button>
                )}
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'16px'}}>
                {[
                  ...
                  {mode:'both' as RecordMode,icon:'🎥',title:'Webcam + Screen',desc:'Screen recording with webcam picture-in-picture'},
                ].map(opt=>(
        ...
        {stage==='preview'&&(
          <div style={{flex:1,display:'flex',flexDirection:'column',padding:'20px',gap:'14px'}}>
            <div style={{flex:1,borderRadius:'12px',overflow:'hidden',position:'relative',background:'#000'}}>
              ...
              {mode==='screen'&&<PreviewMirror videoRef={screenRef} />}
              {mode==='both'&&(
                <>
                  <PreviewMirror videoRef={screenRef} />
                  <div style={{position:'absolute',bottom:'16px',right:'16px',width:'200px',aspectRatio:'16/9',borderRadius:'10px',overflow:'hidden',border:'2px solid rgba(255,255,255,0.6)',boxShadow:'0 4px 16px rgba(0,0,0,0.4)'}}>
              ...
              <div style={{position:'absolute',top:'14px',left:'14px',background:'rgba(0,0,0,0.6)',color:'rgba(255,255,255,0.7)',fontSize:'11px',padding:'4px 12px',borderRadius:'20px'}}>
                Preview — {mode==='both'?'Webcam + Screen (PiP)':mode==='webcam'?'Webcam':'Screen'}
              </div>
            </div>
        ...
        {stage==='review'&&mainUrl&&(
          <div style={{flex:1,display:'flex',flexDirection:'column',padding:'20px',gap:'14px'}}>
            <div style={{flex:1,borderRadius:'12px',overflow:'hidden',background:'#000',position:'relative'}}>
              <video ref={mainPlaybackRef} src={mainUrl} controls={mode!=='both'} muted={mode==='both'} style={{width:'100%',height:'100%',objectFit:'contain'}}
              ...
              />
              {mode==='both'&&pipUrl&&(
                <div style={{position:'absolute',bottom:'12px',right:'12px',width:'160px',aspectRatio:'16/9',borderRadius:'8px',overflow:'hidden',border:'2px solid rgba(255,255,255,0.6)'}}>
                  <video ref={pipPlaybackRef} src={pipUrl} muted style={{width:'100%',height:'100%',objectFit:'cover'}} />
                </div>
              )}
              {mode==='both'&&(
                <button onClick={()=>{if(isPlaying){mainPlaybackRef.current?.pause()}else{mainPlaybackRef.current?.play()}}} style={{position:'absolute',bottom:'12px',left:'12px',padding:'8px 18px',background:'rgba(46,107,74,0.9)',color:'white',border:'none',borderRadius:'8px',fontSize:'12px',fontWeight:'700',cursor:'pointer'}}>
            ...
            <div style={{flex:1,display:'flex',overflow:'hidden'}}>
              <div style={{flex:1,display:'flex',flexDirection:'column',padding:'16px',gap:'12px',overflow:'hidden'}}>
                ...
                <div style={{flex:1,position:'relative',borderRadius:'10px',overflow:'hidden',background:'#000',cursor:activeTool==='annotate'?(placingCaptionZone?'copy':'crosshair'):'default'}}>
                  ...
                  {mode==='both'&&pipUrl&&(
                    <video ref={pipPlaybackRef} src={pipUrl} muted style={{
                      display:pipHidden?'none':'block',
                      position:'absolute',bottom:'16px',right:'16px',width:'180px',aspectRatio:'16/9',
                      borderRadius:'10px',objectFit:'cover',border:'2px solid rgba(255,255,255,0.55)',
                  ...
                  {activeCaptions.length>0&&(
                    <div style={{position:'absolute',left:`${captionZone.x}%`,top:`${captionZone.y}%`,width:`${captionZone.width}%`,minHeight:`${captionZone.height}%`,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:'4px',pointerEvents:'none',zIndex:10}}>
                      {activeCaptions.map(c=>(
                        <div key={c.id} style={{background:'rgba(0,0,0,0.85)',color:'white',padding:'6px 18px',borderRadius:'5px',fontSize:'15px',fontWeight:'600',textAlign:'center',maxWidth:'100%',wordBreak:'break-word'}}>{c.text}</div>
                      ))}
                    </div>
                  )}
                  ...
                  {mode==='both'&&pipHidden&&activeTool==='webcam'&&(
                    <div style={{position:'absolute',bottom:'16px',right:'16px',width:'180px',aspectRatio:'16/9',borderRadius:'10px',border:'2px dashed rgba(255,255,255,0.3)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:5}}>
                      <span style={{color:'rgba(255,255,255,0.4)',fontSize:'10px'}}>Webcam hidden here</span>
                    </div>
                  )}

                  <div style={{position:'absolute',bottom:'10px',right:'12px',background:'rgba(0,0,0,0.65)',color:'white',fontSize:'12px',padding:'3px 10px',borderRadius:'10px',pointerEvents:'none',zIndex:12}}>
                ...
                {activeTool==='webcam'&&mode==='both'&&(
                  <div style={{background:'rgba(255,255,255,0.04)',borderRadius:'8px',padding:'14px',border:'1px solid rgba(255,255,255,0.08)'}}>
                    <div style={{color:'rgba(255,255,255,0.7)',fontSize:'12px',fontWeight:'700',marginBottom:'6px'}}>📹 Webcam PiP — hide the webcam overlay for specific sections</div>
                    <p style={{color:'rgba(255,255,255,0.3)',fontSize:'10px',marginBottom:'10px'}}>Watch the preview above — the webcam circle will disappear and reappear as you scrub through hidden ranges.</p>
        ...
        {stage==='saving'&&(
          <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center'}}>
            <div style={{textAlign:'center'}}>
              ...
              {mode==='both'&&<div style={{color:'rgba(255,255,255,0.3)',fontSize:'11px',marginTop:'8px'}}>Saving screen and webcam as two linked files</div>}
            </div>
          </div>
        )}
        ...
        {stage==='done'&&(
          <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center'}}>
            <div style={{textAlign:'center',maxWidth:'460px'}}>
              ...
              <div style={{color:'rgba(255,255,255,0.4)',fontSize:'13px',marginBottom:'20px'}}>"{title}"{mode==='both'?' (screen + webcam)':''}</div>
              {wasDownloaded&&(
                <div style={{background:'rgba(196,122,44,0.15)',border:'1px solid rgba(196,122,44,0.3)',borderRadius:'8px',padding:'10px 16px',marginBottom:'24px',fontSize:'12px',color:'#C47A2C',lineHeight:'1.7'}}>
                  Supabase Storage isn't configured yet so the video{mode==='both'?'s':''} downloaded to your computer. We'll set that up as a next step.
                </div>
              )}
